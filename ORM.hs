module ORM(Entity, insert, processingResult, tableName, idColumnName, allColumns, imdbid, bacon, readSql, asSql) where
...
class Eq a => Entity a where
    ...
    insert entity conn = do
        stmt <- prepare conn $ saveQuery entity
